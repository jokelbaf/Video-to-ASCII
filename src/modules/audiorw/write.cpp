...
void audiorw::write(
  ...
  if (!(format_context->oformat = av_guess_format(nullptr, filename.c_str(), nullptr))) {
    cleanup(codec_context, format_context, resample_context, frame, packet);
    throw std::runtime_error(
  ...
  AVStream * stream;
  if (!(stream = avformat_new_stream(format_context, nullptr))) {
    cleanup(codec_context, format_context, resample_context, frame, packet);
    throw std::runtime_error(
  ...
  if ((error = avcodec_open2(codec_context, output_codec, nullptr)) < 0) {
    cleanup(codec_context, format_context, resample_context, frame, packet);
    av_strerror(error, errbuf, errbuf_size);
    throw std::runtime_error(

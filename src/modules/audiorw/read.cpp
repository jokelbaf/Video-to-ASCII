...
std::vector<std::vector<double>> audiorw::read(
  ...
  int error = avformat_open_input(&format_context, filename.c_str(), nullptr, nullptr);
  if (error != 0) {
  ...
  while (sample < end_sample) {
    ...
    while ((error = avcodec_receive_frame(codec_context, frame)) == 0) {
      ...
      auto* audio_data_ = reinterpret_cast<uint8_t*>(audio_data.data());
      const auto ** frame_data = const_cast<const uint8_t**>(frame -> extended_data);
      if ((error = swr_convert(
        resample_context,
        &audio_data_,
        frame -> nb_samples,
        frame_data  ,
